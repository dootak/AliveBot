...
def extract_level(tree):
    ...
    levelText = level.find('.//*[@class="u-vertical-center"]')
    return int(levelText.text_content().strip())


def extract_endorsement(tree):
    endorsementDiv = tree.find('.//*[@class="endorsement-level"]')
    endorsement = endorsementDiv.xpath("//div[@class='u-center']")
    shotcaller = endorsementDiv.xpath('.//*[contains(@class, "shotcaller")]')
    shotcaller = float(shotcaller[0].get('data-value')) if shotcaller else 0.0

    teammate = endorsementDiv.xpath('.//*[contains(@class, "teammate")]')
    teammate = float(teammate[0].get('data-value')) if teammate else 0.0

    sportsmanship = endorsementDiv.xpath('.//*[contains(@class, "sportsmanship")]')
    sportsmanship = float(sportsmanship[0].get('data-value')) if sportsmanship else 0.0

    return {
        'level': int(endorsement[0].text_content().strip()),
